...
<div id="tambah-keranjang-modal-{{ $produk->id }}" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl overflow-hidden w-full max-h-[90vh] flex flex-col">
            ...
            <div
                ...
                <div class="relative flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-xs font-semibold text-blue-100 uppercase tracking-wider mb-1">Tambah
                                Keranjang
                            </p>
                            <h3 class="text-xl md:text-2xl font-bold text-white leading-tight">Tambahkan produk ke
                                keranjang</h3>
                        </div>
                    </div>
                    <button type="button"
                        ...
                        data-modal-hide="tambah-keranjang-modal-{{ $produk->id }}">
            ...
            <div class="p-4 border-t bg-gray-50 flex flex-col sm:flex-row gap-3 justify-end">
                ...
                <form id="formTambahKeranjang" method="POST" action="{{ route('user.produk.tambahKeranjang') }}"
                    ...
                    <input type="hidden" name="subtotal" id="formTambahSubtotal" value="">
                    <input type="text" name="jumlah" id="formTambahJumlah" value="1"
                        style="display:none;">
                    <button
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // ==================== VARIAN CARD BORDER HIGHLIGHT ====================
        var radios = document.querySelectorAll('input[name="varian_id"]');

        function updateVarianCardHighlight() {
            radios.forEach(function(radio) {
                var label = radio.closest('.varian-card-label') || document.querySelector(
                    'label[for="' + radio.id + '"]');
                if (label) {
                    if (radio.checked) {
                        label.classList.add('varian-selected');
                    } else {
                        label.classList.remove('varian-selected');
                    }
                }
            });
        }
        radios.forEach(function(radio) {
            radio.addEventListener('change', updateVarianCardHighlight);
        });
        updateVarianCardHighlight();
        setTimeout(updateVarianCardHighlight, 10);

        // ==================== SUBTOTAL & IMAGE UPDATE + FORM SYNC ====================
        ...
        // Form hidden input sync
        var formVarianId = document.getElementById('formTambahVarianId');
        var formJumlah = document.getElementById('formTambahJumlah');
        var formSubtotal = document.getElementById('formTambahSubtotal');

        function getCheckedVarianId() {
            var checked = document.querySelector('input[name="varian_id"]:checked');
            return checked ? checked.value : '';
        }

        function getJumlah() {
            return parseInt(jumlahInput.value) || 1;
        }

        function updateSubtotalAndForm() {
            var harga = 0;
            ...
            // Sync hidden inputs
            if (formVarianId) formVarianId.value = varianId;
            if (formJumlah) formJumlah.value = jumlah;
            if (formSubtotal) formSubtotal.value = subtotal;
        }
